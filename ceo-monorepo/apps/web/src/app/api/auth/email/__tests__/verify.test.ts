...
describe('POST /api/auth/email/verify', () => {
  ...
  it('should verify valid token and update user', async () => {
    ...
    (prisma.emailVerification.findUnique as jest.Mock).mockResolvedValueOnce(mockVerification);
    (prisma.user.update as jest.Mock).mockResolvedValueOnce({
      ...mockUser,
      emailVerified: true,
    });
    (prisma.emailVerification.delete as jest.Mock).mockResolvedValueOnce({});
    (prisma.emailVerificationAttempt.deleteMany as jest.Mock).mockResolvedValueOnce({
      count: 0,
    });
  ...
  it('should reject invalid token', async () => {
    (prisma.emailVerification.findUnique as jest.Mock).mockResolvedValueOnce(null);
  ...
  it('should reject expired token', async () => {
    ...
    (prisma.emailVerification.findUnique as jest.Mock).mockResolvedValueOnce(mockVerification);
    (prisma.emailVerification.delete as jest.Mock).mockResolvedValueOnce({});
    (prisma.emailVerificationAttempt.findUnique as jest.Mock).mockResolvedValueOnce(null);
    (prisma.emailVerificationAttempt.upsert as jest.Mock).mockResolvedValueOnce({
      email: 'test@example.com',
      attempts: 1,
      updatedAt: new Date(),
    });
  ...
  it('should track verification attempts and enforce max attempts', async () => {
    ...
    (prisma.emailVerificationAttempt.findUnique as jest.Mock).mockResolvedValueOnce({
      email: 'test@example.com',
      attempts: 3,
      updatedAt: new Date(),
    });
  ...
  it('should reset attempts on successful verification', async () => {
    ...
    expect(
      (prisma.emailVerificationAttempt.deleteMany as jest.Mock).mock.calls.some(
        (call) => call[0].where.email === 'test@example.com'
      )
    ).toBe(true);
  });
