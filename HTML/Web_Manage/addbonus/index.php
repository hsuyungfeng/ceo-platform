...
where 1 $sqlstr and car.timeend < '".date("Y-m-d H:i:s")."' and tbonus = 0 and $table.isshow != 4 and $table.isshow = 3 order by $table.id desc";
//echo $sql;
//echo request_cd(); 
...
<a href="add.php" class="btn-slide">新增</a></p>-->
        ...
        <select id="stype" name="stype">
            ...
            <option value="0" <?php echo ($stype=="0") ? "selected" : ""?>>團購藥名</option>
...
<input name="sdate" type="text" id="sdate" onfocus="HS_setDate(this)" value="<?php echo $ssdate?>" size="12" maxlength="12" />
	...
	<th width="120" align="center" >團購藥名</th>
...
?>
	<tr>
      ...
      <td align="center"><?=$row["bonus3"]?></td>
      
      <td align="center"> <?php if($row["createtime2"] && empty($row["createtime3"])){echo "可出貨";}else{if(date("Y-m-d H:i:s")<$row["pid_timeend"] && $row["isshow"]!=2 && $row["isshow"]!=3 && $row["isshow"]!=4){echo "進行中";}else{echo $aryYN3[$row["isshow"]];}}?></td>
      
      <td align="center"><?php echo $row["bonus3"]-ceo_range($row["pid"],$row["pid_total"])*$row["amount"];?></td>
...
<?php
if($row["tbonus"] != 1){
$bonus = $row["bonus3"] - ceo_range($row["pid"],$row["pid_total"])*$row["amount"];
$sql2 = "UPDATE `ceo_user` SET  `bonus` =  `bonus`+".$bonus." WHERE  `ceo_user`.`id` =".$row["uid_id"]."";
//echo $sql2."<BR>";
$sql3 = "UPDATE `ceo_order` SET `tbonus` =  1 WHERE  `ceo_order`.`id` =".$row["id"]."";
//echo $sql3."<BR>";
mysql_query($sql2);
mysql_query($sql3);
}
...
$db->close();
script("購物金產生完成!","../order/index.php");
?>
	</tbody>
		<tfoot>
	<tr>
