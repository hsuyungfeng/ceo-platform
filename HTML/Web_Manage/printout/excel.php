...
foreach($rows as $row)
{
	...
	if($row["isshow"]!=3)
	{
	$sql3 = "UPDATE `ceo_order` SET `bonus3` =  ".$sum." WHERE  `ceo_order`.`id` =".$row["id"]."";	
	//echo $sql3."歷史銷售額單筆<BR>";
	mysql_query($sql3);
	}
 ...
 if($uid != $row["uid"] && $uid!=""){
	...
	if($row["isshow"]!=3)
	{
	$sql3 = "UPDATE `ceo_order` SET  `bonus2` =  ".$tsum." WHERE  `ceo_order`.`id` =".$oid."";	
...
?>
	<tr>
      ...
      <td align="center"><?php echo $sum; $zsum = $sum; ?></td>
	...
	//mysql_query($sql2);
		...
		$data["isshow"]=3;
		//更新狀態
		$sn=$db->query_update($table,$data,"id=".$row["id"]."");
		?>
